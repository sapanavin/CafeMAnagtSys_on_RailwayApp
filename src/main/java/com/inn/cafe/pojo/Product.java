...
@NamedQuery(name = "Product.getProductByCategory", query = "select new com.inn.cafe.wrapper.ProductWrapper(p.id,p.name) from Product p where p.category.id=:id")                 
...
@NamedQuery(name = "Product.getMaxProductId", query ="select MAX(id) from Product p")
...
//Hibernate: select product.id , product.name, product.description, product.price, product.status from product product
